...
const flsedetec = { v: "2.0.3", channel: "stable" }; // Info about the version and update channel //
const settings = {}; // Language info is stored here //
var imports = {}; // Not used yet, just an empty object //
try { 
  flsestrings; // Checks if the flsestrings already exist //
} catch (error) { 
  var flsestrings = {}; // If not, create an empty object called flsestrings //
}
flseBootstrap(); // Start the setup function //

function flseBootstrap() { // Start the setup function //
  // Get browser language like "en-US" and change "-" to "_" so it becomes "en_US" //
  settings["longLan"] = navigator.language.replace("-", "_");
  // If there's an underscore, take just the first part (like the "en" from "en_US") //
